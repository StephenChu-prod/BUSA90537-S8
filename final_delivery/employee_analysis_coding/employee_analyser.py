...
class EmployeeAnalyser:
    ...
    def quarterly_performance(self):
        ...
        If 'Role' column exists, benchmark against others with same role.
        Otherwise, benchmark against overall population.
        """
        data_add1 = self.data.copy()

        # Check if 'Role' column exists
        if 'Role' in data_add1.columns:
            # Group by Quarter and Role to get role-level median
            role_quarterly = data_add1.groupby(['Quarter', 'Role'])['Hours Worked'].agg(
                overall_median='median').reset_index()

            # Group by Employee, Quarter, and Role to get employee-level median
            employee_quarterly = data_add1.groupby(['Quarter', 'Employee', 'Role'])['Hours Worked'].agg(
                median='median').reset_index()

            # Merge employee with role-based overall
            quarterly_performance = pd.merge(employee_quarterly, role_quarterly,
                                         how='left', on=['Quarter', 'Role'])

        else:
            # No 'Role', fallback to overall median across all employees
            employee_quarterly = data_add1.groupby(['Quarter', 'Employee'])['Hours Worked'].agg(
                median='median').reset_index()

            overall_quarterly = data_add1.groupby(['Quarter'])['Hours Worked'].agg(
               overall_median='median').reset_index()

            # Merge employee with overall
            quarterly_performance = pd.merge(employee_quarterly, overall_quarterly,
                                         how='left', on='Quarter')

        # Calculate deviation from benchmark (role-based or overall)
